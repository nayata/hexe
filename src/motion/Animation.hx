...
class Animation {
	...
	public function event(name:String, time:Int):Bool {
		...
		if (event == null) {
			...
			event.type = "event";
			event.start = Quantize.frame(time);
	...
	public function calculate(frame:Frame) {
		...
		if (key == null) {
			...
			frame.start = Quantize.frame(frame.time);
			frame.end = Quantize.frame(frame.time);
		...
		if (key != null) {
			...
			frame.start = Quantize.frame(frame.time);
			frame.end = Quantize.frame(key.time);
	...
	inline function cmp(a:Frame, b:Frame):Int {
