...
class Tracker extends h2d.Object {
	...
	public function new(parent:h2d.Object) {
		...
		highlight = new h2d.ScaleGrid(Assets.icon("selection"), 2, 2, this);
		highlight.visible = false;
		highlight.smooth = false;

		channel.set("event", new Track(0xdb1a40, 0, this));

		channel.set("x", new Track(0x00c3ff, height, this));
		channel.set("y", new Track(0x00c3ff, height * 2, this));

		channel.set("scaleX", new Track(0x00c3ff, height * 3, this));
		channel.set("scaleY", new Track(0x00c3ff, height * 4, this));

		channel.set("rotation", new Track(0x00c3ff, height * 5, this));
		channel.set("alpha", new Track(0x00c3ff, height * 6, this));
	}
		...
		for (track in channel) {
			for (keyframe in track.keyframe) {
				if (keyframe.frame != null) {
					keyframe.x = keyframe.frame.start * scaling;
				}
			}
		}
		liner.update();
