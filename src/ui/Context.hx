package ui;

import hxd.Direction;
import h2d.col.Point;
import h2d.Object;
import h2d.Bitmap;
import hxd.Event;

import ui.Item;


class Context extends h2d.Object {
	var editor:Editor;
	
	var input:h2d.Interactive;

	var panel:h2d.Bitmap;
	var highlight:h2d.Bitmap;
	
	var items:Array<Item> = [];
	var selected:Null<Item> = null;

	public var attachment = hxd.Direction.Up;

	public var width:Int = 240;
	public var height:Int = 40;

	public var dividers:Int = 0;
	public var padding:Int = 0;

	public var size:Int = 40;

	
	public function new(?parent:h2d.Object) {
		super(parent);

		editor = Editor.ME;

		input = new h2d.Interactive(width, size, this);
		input.cursor = Default;
		input.onClick = onClick;
		input.onMove = onMove;
		input.onOut = onOut;

		panel = new h2d.Bitmap(h2d.Tile.fromColor(Style.menu, width, size), this);
		panel.smooth = false;

		highlight = new h2d.Bitmap(h2d.Tile.fromColor(Style.over, width, size), panel);
		highlight.visible = false;

		panel.filter = new h2d.filter.DropShadow(5, Math.PI/4, 0, 0.25, 20.0, 1, 1.0, true);

		visible = false;
	}


	public function add(value:String, ?icon:String = "", ?shortcut:String = "", ?closable:Bool = true) {
		var item = new Item(this, value);
		
		item.y = padding + dividers + items.length * height;
		item.closable = closable;
		item.shortcut = shortcut;
		item.icon = icon;

		items.push(item);

		onResize();
	}


	public function get(name:String) {
		for (node in items) {
			if (node.text == name) return node;
		}
		return null;
	}


	public function addDivider() {
		var item = new Item(this, "");

		item.y = padding + dividers + items.length * height;
		item.setDivider(width, 4);

		dividers += 4;
	}


	function onClick(e:hxd.Event) {
		if (!visible) return;

		if (selected != null) {
			onChange(selected.text, selected.icon);
			if (selected.closable) close();
		}
	}


	function onMove(event:hxd.Event) {
		if (!visible) return;

		selected = null;

		var mouse = new h2d.col.Point(event.relX, event.relY);

		for (node in items) {
			if (mouse.y >= node.y && mouse.y < node.y + size) {
				selected = node;
			}
		}

		highlight.visible = selected != null ? true : false;
		highlight.y = selected != null ? selected.y : 0;
	}


	function onOut(e:hxd.Event) {
		close();
	}


	public function open(?prop:String) {
		x = editor.s2d.mouseX - 20;
		y = editor.s2d.mouseY - 5;

		if (attachment == hxd.Direction.Down) y = editor.s2d.mouseY - input.height + height * 0.5;

		if (x + width > editor.WIDTH) x = editor.WIDTH - width - 20;
		if (y + input.height > editor.HEIGHT) y = editor.HEIGHT - input.height - 20;
		if (x < 20) x = 20;
		
		visible = true;
	}


	public function close() {
		selected = null;
		highlight.visible = false;
		visible = false;
	}


	function onChange(value:String, type:String) {
	}


	function onResize() {
		input.width = width;
		input.height = padding + dividers + items.length * height;

		panel.width = input.width;
		panel.height = input.height;
	}
}